...
const DataLoader = {
    ...
    async loadLocalProjects() {
        return new Promise((resolve, reject) => {
            ...
            xhr.open('GET', './wangzhan/admin/projects.json', true);
                    ...
                    try {
                        ...
                        if (isFileProtocol && data) {
                            data.forEach(project => {
                                if (project.url && project.url.startsWith('./') && !project.url.startsWith('./wangzhan/')) {
                                    project.url = project.url.replace('./', './wangzhan/');
                                }
                            });
                        }
    ...
    async loadLocalCategories() {
        return new Promise((resolve, reject) => {
            const xhr = new XMLHttpRequest();
            xhr.open('GET', './wangzhan/admin/categories.json', true);
        ...
        const embeddedProjects = [
            {
                ...
                title: "博客",
                description: "记录美好日常",
                url: isFileProtocol ? "./wangzhan/blog.html" : "./blog.html",
            ...
            },
            {
                ...
                title: "相册",
                description: "珍藏美好回忆",
                url: isFileProtocol ? "./wangzhan/photo.html" : "./photo.html",
            ...
            },
            {
                ...
                title: "留言板",
                description: "记录用户留言",
                url: isFileProtocol ? "./wangzhan/lyb.html" : "./lyb.html",
...
const ProjectRenderer = {
    ...
    createProjectCard(project, options = {}) {
        ...
        // 处理项目URL，确保在不同环境下都能正确跳转
        let projectUrl = project.url || '#';
        
        // 检查是否是通过file://协议访问的
        const isFileProtocol = window.location.protocol === 'file:';
        
        // 如果是本地文件访问且URL是相对路径，添加/wangzhan前缀
        if (isFileProtocol && projectUrl.startsWith('./') && !projectUrl.startsWith('./wangzhan/')) {
            projectUrl = projectUrl.replace('./', './wangzhan/');
        }
        ...
        if (imageUrl.startsWith('img/') || imageUrl.startsWith('svg/')) {
            imageUrl = `./wangzhan/static/${imageUrl}`;
        } else if (imageUrl.startsWith('./static/img/')) {
            // 修正已有的./static/img/路径
            imageUrl = imageUrl.replace('./static/', './wangzhan/static/');
        }
